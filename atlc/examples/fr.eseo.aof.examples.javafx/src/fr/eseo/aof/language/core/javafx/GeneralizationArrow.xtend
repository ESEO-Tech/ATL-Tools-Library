...
class GeneralizationArrow extends Group {
	...
	new() {
		...
		this.children.add(head4)
		this.children.add(head1)
		this.children.add(head2)
		this.children.add(head3)
	...
	private def updateHead() {
		...
		head3.startX = x
		head3.startY = y
		head4.startX = head1.startX
		head4.startY = head1.startY
		head4.endX = head3.startX + (0.5) * (head3.endX - head3.startX)
		head4.endY = head3.startY + (0.5) * (head3.endY - head3.startY)
	}

	override toString() {
		'''Generalization Arrow[fromX=«line.startX», fromY=«line.startY», toX=«line.endX», toY=«line.endY»]'''
	}
